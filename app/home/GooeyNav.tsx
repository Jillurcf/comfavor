...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  // randomness is intentional for animation variance, called from event handlers only
  ...
  const makeParticles = (element: HTMLElement) => {
    ...
    const r = particleR;
    const bubbleTime = animationTime * 2 + timeVariance + noise(timeVariance * 2);
  ...
  return (
    <>
      ...
      <div className="gooey-nav relative" ref={containerRef}>
        <nav className="flex relative" style={{ transform: 'translate3d(0,0,0.01px)' as const }}>
          <ul
            ref={navRef}
          ...
          >
            {items.map((item, index) => (
              <li
                key={index}
                className={`rounded-full relative cursor-pointer transition-[background-color,color,box-shadow] duration-300 ease text-white ${
              ...
              >
                <a
                  ...
                  className="outline-none py-[0.6em] px-[1em] inline-block text-sm tracking-wide"
                >
                  {item.label}
                </a>
              </li>
