...
const OptionWheel = ({
  ...
  const frameFnRef = useRef<((now: number) => void) | null>(null);
  ...
  }, [items, fontSize, spacing, curve, tilt, blur, fade, minOpacity, side, loop, smoothing, draggable, remPx]);

  const positionItems = useCallback((pos: number) => {
    ...
    >
      {items.map((label, index) => (
        <div
          ...
          className={`absolute top-1/2 cursor-pointer whitespace-nowrap leading-none will-change-[transform,opacity,filter] [font-size:14px] [color:color-mix(in_srgb,var(--ow-active-color)_calc(var(--ow-p,0)*100%),var(--ow-text-color))] ${
            side === 'right'
